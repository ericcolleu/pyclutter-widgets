...
from widget.thumbnail_menu import ThumbnailPage

def on_input(stage, event, thumbnailpage):
	if event.keyval == keysyms.Left:
		thumbnailpage.select((thumbnailpage.get_selected()+1)%4)
...
def main(image_directory):
	...
	thumbnailpage = ThumbnailPage(size=(1024,768), max_row=5, max_column=5)
	stage.add(thumbnailpage)
	n_items = len(item_images)
	items = []
	stage.show()
	for image in item_images:
		thumbnailpage.add(clutter.Texture(image))
